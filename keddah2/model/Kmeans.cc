...
void Kmeans::SetStartTime(int s) {
	start_time = s;
}
int Kmeans::hdfs_r_nofcontainers() {
	double mean = 28.44;
	double sd = 9.6;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);

}
int Kmeans::hdfs_r_nofsrcpercont() {
	double mean = 1.2;
	double sd = 0.01;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::hdfs_r_nofflowspersrc() {
	double mean = 1.22;
	double sd = 0.61;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::hdfs_r_sizeofflow() {
	double mean = 128;
	double sd = 0.1;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::hdfs_r_starttimeofflow() {
	double mean = 287.39;
	double sd = 312.42;
	return JobPara::GetNormalRamdonVariableL1(mean, sd) / 1000; // in second
}

int Kmeans::shuffle_nofcontainers() {
	double mean = 1.85;
	double sd = 1.06;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::shuffle_nofsrcpercont() {
	double mean = 14.0;
	double sd = 0.46;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::shuffle_nofflowspersrc() {
	double mean = 2.65;
	double sd = 2.14;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::shuffle_sizeofflow() {
	double mean = 22.24;
	double sd = 12.12;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::shuffle_starttimeofflow() {
	double mean = 295;
	double sd = 304;
	return JobPara::GetNormalRamdonVariableL1(mean, sd) / 1000; // in second
}
int Kmeans::hdfs_w_nofcontainers() {
	return 1;
}
int Kmeans::hdfs_w_nofdstpercont() {
	double mean = 13.31;
	double sd = 0.82;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::hdfs_w_nofflowspersrc() {
	double mean = 4.95;
	double sd = 3.43;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::hdfs_w_sizeofflow() {
	double mean = 118;
	double sd = 22;
	return JobPara::GetNormalRamdonVariableL1(mean, sd);
}
int Kmeans::hdfs_w_starttimeofflow() {
	double mean = 346;
	double sd = 312;
	return JobPara::GetNormalRamdonVariableL1(mean, sd) / 1000; // in second
}
...
void Kmeans::start() {
	std::cout << "Job kmeans strted " << this->jobid << " "
	...
	for (int i = 0; i < total_mapper; i++) {
		...
		Ptr<SystemThread> mapper = Create<SystemThread>(
				MakeCallback(&Kmeans::Map_containers_thread, this));
		sleep(1); // don't flush threads together
		mapperthrds.push_front(mapper);
		mapper->Start();

		std::cout << "Mapper " << i << " from job " << this->jobid
				<< " started " << Simulator::Now().GetSeconds() << std::endl;
		if (reducerstarted <= total_reducer
				&& ((float) this->finished_mappers) / total_mapper
						>= CONFIG::REDUCERSTARTRATE) {
			// start reducer container

			for (; reducerstarted < total_reducer; reducerstarted++) {

				Ptr<SystemThread> reducer = Create<SystemThread>(
						MakeCallback(
								&Kmeans::Reducer_containers_thread,
								this));
				sleep(1); // don't flush threads together
				reducerthrds.push_front(reducer);
				reducer->Start();

				std::cout << "Reducer " << reducerstarted << " from job "
						<< this->jobid << " started "
						<< Simulator::Now().GetSeconds() << std::endl;
			}
		}
	...
	for (; reducerstarted < total_reducer; reducerstarted++) {
		...
		Ptr<SystemThread> reducer = Create<SystemThread>(
				MakeCallback(&Kmeans::Reducer_containers_thread,
						this));
		sleep(1); // don't flush threads together
		reducerthrds.push_front(reducer);
		reducer->Start();
		std::cout << "Reducer " << reducerstarted << " from job " << this->jobid
				<< " started " << Simulator::Now().GetSeconds() << std::endl;
	}
...
void Kmeans::Map_containers_thread() {
	...
	for (int i = 0; i < src; i++) // HDFS read from src to this node
		...
		Ptr<Node> from = HadoopYarn::getHosts().Get(
				rand() % HadoopYarn::getHosts().GetN()); // random a source to send the traffic
						// Ptr<Node> from = HadoopYarn::getHosts().Get(1);
						//	std::cout << "flows " << flows << std::endl;
		for (int j = 0; j < flows; j++) {
			Address sinkAddress(
					InetSocketAddress(
							to->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(),
							50010));
			Ptr<Socket> ns3TcpSocket = Socket::CreateSocket(from,
					TcpSocketFactory::GetTypeId());
			Ptr<MyApp> app = CreateObject<MyApp>();
			app->Setup(ns3TcpSocket, sinkAddress, 1000,
	...
	for (std::set<Ptr<MyApp> >::iterator it = apps.begin(); it != apps.end();
			it++) {
		while (!((*it)->Finished())) {
	...
	std::cout << "Mappper " << this->finished_mappers << " on node " << node
			<< " from job " << this->jobid << " finished at "
			<< Simulator::Now().GetSeconds() << std::endl;
}
void Kmeans::Reducer_containers_thread() {
	// sending traffic
	int node = YarnScheduler::YarnScheduler_container_started();
	int src = this->shuffle_nofsrcpercont();
	Ptr<Node> to = HadoopYarn::getHosts().Get(node);
	std::cout << "Shuffle " << this->finished_reducers << " on node " << node
			<< " from job " << this->jobid << "  started with " << src
			<< " flows " << Simulator::Now().GetSeconds() << std::endl;
	std::set<Ptr<MyApp> > apps;
	for (int i = 0; i < src; i++) {
		int flows = this->shuffle_nofflowspersrc();
		Ptr<Node> from = HadoopYarn::getHosts().Get(
				this->mapperlocation.at(rand() % this->mapperlocation.size()));
		for (int j = 0; j < flows; j++) {
			Address sinkAddress(
					InetSocketAddress(
							to->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal(),
	...
	for (int i = 0; i < this->NumReplicas - 1; i++) { // one copy is local
		Ptr < Node > to = HadoopYarn::getHosts().Get(
				rand() % HadoopYarn::getHosts().GetN());
		int flows = this->hdfs_w_nofflowspersrc();
		std::cout << "HDFS write " << this->finished_reducers << " on node "
				<< node << " from job " << this->jobid << "  write to  "
				<< to->GetId() << " fwith " << flows << " flows "
				<< Simulator::Now().GetSeconds() << std::endl;
		for (int j = 0; j < flows; j++) {
			Address sinkAddress(
					InetSocketAddress(
			...
			from->AddApplication(app);
			app->SetStartTime(
					Seconds(this->hdfs_w_starttimeofflow() + this->start_time));
			apps.insert(app);
		}
		from = to;
	}
	...
	std::cout << "HDFS write " << this->finished_hdfswrite << " on node "
			<< node << " from job " << this->jobid << " finished at "
			<< Simulator::Now().GetSeconds() << std::endl;
}
int Kmeans::is_job_finished() {
	return job_finished;
}
